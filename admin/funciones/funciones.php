<?php
	...
	function VerificaExtensionFilesAdmin($FILE)
	{
		...
		if ($encontrada) {
			echo "<script>alert('Formato de Archivo Incompatible'); location.href='?sw=home_landing_2020';</script>";
			exit();
		}
		...
		if (count($arreglo_archivo) <> 2) {
			echo "<br>archivo invalido con mas de una extension";
			echo "    <script>alert('Formato de Archivo Incompatible'); 			location.href='?sw=home_landing';    </script>";
			exit();
		}
	...
	function FormularioImparticion($PRINCIPAL, $id_empresa, $id_imparticion)
	{
		...
		if ($datos_imparticion[0]->id_modalidad_imparticion == "2") {
			$select_modalidad_02 = " selected ";
		}
		if ($datos_imparticion[0]->id_modalidad_imparticion == "3") {
			$select_modalidad_03 = " selected ";
		}
		if ($datos_imparticion[0]->id_modalidad_imparticion == "4") {
			$select_modalidad_04 = " selected ";
		}
		if ($datos_imparticion[0]->id_modalidad_imparticion == "5") {
			$select_modalidad_05 = " selected ";
		}
		if ($datos_imparticion[0]->id_modalidad_imparticion == "6") {
			$select_modalidad_06 = " selected ";
		}
		if ($datos_imparticion[0]->id_modalidad_imparticion == "7") {
			$select_modalidad_07 = " selected ";
		}
		...
		if ($datos_imparticion[0]->valor_curso == "SI") {
			$select_costos_asociados_SI = " selected ";
		}
		if ($datos_imparticion[0]->valor_curso == "NO") {
			$select_costos_asociados_NO = " selected ";
		}
	...
	function ListadoMallasAdmin1($PRINCIPAL, $id_empresa, $excel, $id_curso, $llamada)
	{
		...
		foreach ($mallas as $unico) {
			...
			elseif ($llamada == "imparticion_solo_preenciales") {
	...
	function VerificaFotoPersonalDesdeAdmin($rut)
	{
		...
		elseif (file_exists($ruta_admin . ".JPG")) {
			$imagen = $ruta_admin . ".JPG";
		}
		elseif (file_exists($ruta_admin . ".gif")) {
		...
		elseif (file_exists($ruta . ".gif")) {
			$imagen = $ruta . ".gif";
		}
		elseif (file_exists($ruta_rut_completo . ".jpg")) {
			$imagen = $ruta_rut_completo . ".jpg";
		}
		elseif (file_exists($ruta_rut_completo . ".JPG")) {
			$imagen = $ruta_rut_completo . ".JPG";
		}
		elseif (file_exists($ruta_rut_completo . ".gif")) {
	...
	function saveDatosGeneralesReembolso($reembo_numdoc, $reembo_tipdocid, $reembo_tipdocOtro, $reembo_proveerut, $reembo_servid, $reembo_servotro, $reembo_montoto, $reembo_fecemision, $reembo_ota, $reembo_numota, $reembo_cuenta, $reembo_proyecto, $reembo_curso, $reembo_otanombre, $reembo_cui, $reembo_respgast, $reembo_observacion, $reembo_mes, $reembo_anual, $reembo_status, $reemboa_proveenom, $rut_created, $reembo_cuenta_nombre)
	{
		...
		if (count($consulNumDocReemb) == 0) {
			...
			return saveDatosGeneralesReembolsoData($reembo_numdoc, $reembo_tipdocid, $reembo_tipdocOtro, $reembo_proveeid, $reembo_proveerut, $reembo_servid, $reembo_servotro, $reembo_montoto, $reembo_fecemision, $reembo_ota, $reembo_numota, $reembo_cuentaid, $reembo_cuenta, $reembo_proyectid, $reembo_proyecto, $reembo_curso, $reembo_otanombre, $reembo_cui, $reembo_respgast, $reembo_observacion, $reembo_mes, $reembo_anual, $reembo_status, $reemboa_proveenom, $rut_created, $reembo_cuenta_nombre);
		}
		else {
			$result = array("code" => 0, "mensage" => "Número de documento ya existe");
		}
	...
	function updateDatosGeneralesFactura($id, $factur_numdoc, $factur_tipdocid, $factur_proveerut, $factur_servid, $factur_servotro, $factur_montoto, $factur_montonet, $factur_impuest, $factur_fecemision, $factur_ota, $factur_numota, $factur_cuenta, $factur_proyecto, $factur_curso, $factur_otanombre, $factur_cui, $factur_respgast, $factur_observacion, $factur_mes, $factur_anual, $factur_status, $factura_proveenom, $factur_cuenta_nombre)
	{
		//Se validan los datos recibidos antes de ser guardados
		//Se ubica el id del proveedor segun el rut suministrado
		$consulOtec = DatosOtecDadoRut($factur_proveerut);
		...
		$factur_cui = Decodear3($factur_cui);
		$factur_respgast = Decodear3($factur_respgast);
		updateDatosGeneralesFacturaData($id, $factur_numdoc, $factur_tipdocid, $factur_proveeid, $factur_proveerut, $factur_servid, $factur_servotro, $factur_montoto, $factur_montonet, $factur_impuest, $factur_fecemision, $factur_ota, $factur_numota, $factur_cuentaid, $factur_cuenta, $factur_proyectid, $factur_proyecto, $factur_curso, $factur_otanombre, $factur_cui, $factur_respgast, $factur_observacion, $factur_mes, $factur_anual, $factur_status, $factura_proveenom, $factur_cuenta_nombre);
	}
	
	function updateDatosGeneralesReembolso($id, $reembo_numdoc, $reembo_tipdocid, $reembo_tipdocOtro, $reembo_proveerut, $reembo_servid, $reembo_servotro, $reembo_montoto, $reembo_montonet, $reembo_impuest, $reembo_fecemision, $reembo_ota, $reembo_numota, $reembo_cuenta, $reembo_proyecto, $reembo_curso, $reembo_otanombre, $reembo_cui, $reembo_respgast, $reembo_observacion, $reembo_mes, $reembo_anual, $reembo_status, $reemboa_proveenom, $reembo_cuenta_nombre)
	{
		//Se validan los datos recibidos antes de ser guardados
		$reembo_proveeid = $reembo_proveerut;
		...
		updateDatosGeneralesReembolsoData($id, $reembo_numdoc, $reembo_tipdocid, $reembo_tipdocOtro, $reembo_proveeid, $reembo_proveerut, $reembo_servid, $reembo_servotro, $reembo_montoto, $reembo_montonet, $reembo_impuest, $reembo_fecemision, $reembo_ota, $reembo_numota, $reembo_cuentaid, $reembo_cuenta, $reembo_proyectid, $reembo_proyecto, $reembo_curso, $reembo_otanombre, $reembo_cui, $reembo_respgast, $reembo_observacion, $reembo_mes, $reembo_anual, $reembo_status, $reemboa_proveenom, $reembo_cuenta_nombre);
	}
    ...
    */
	...
	function VerificaSQLInjectionLight($valor)
	{
		if (strtoupper(trim($valor)) != "") {
			$valor = str_replace('--', "", $valor);
			$valor = str_replace('.php', "", $valor);
